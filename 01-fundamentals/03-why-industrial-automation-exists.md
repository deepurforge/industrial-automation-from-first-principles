...
- Every diagram above is written in **Mermaid**, which GitHub renders natively — no image export needed.
- The photographs are hot-linked to Wikimedia Commons with credit lines intact; for a fully offline repository, download the originals into `assets/images/` and switch to relative paths.
- Section 16 (tank-level control) is the strongest candidate in this chapter for a companion animated `.html` file, following the same pattern as Chapter 02's `02-where-did-industrial-automation-begin-history-2.html` — happy to build that next if useful.
